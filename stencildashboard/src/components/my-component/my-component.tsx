...
export class MyComponent {
  @Prop() dayNames = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  @Prop() monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'
  ];
  ...
  render() {
    ...
    return (
      <div class="calendar material">
        ...
        <div class="brown">
          <b>{this.selectedDate?.day ? 'Selected Date : ' : ''}{this.selectedDate?.day}
            {this.selectedDate?.month ? (' / ' + this.selectedDate?.month) : ''}{this.selectedDate?.year ? (' / ' + this.selectedDate?.year) : ''}</b>
        </div>
      </div>
    );
  }
